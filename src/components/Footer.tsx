...
export function Footer() {
  return (
    <footer className="bg-background-dark border-t border-white/5 py-16 px-6">
        ...
        <div>
          ...
          <ul className="space-y-4 text-sm text-slate-500">
            {solutionsLinks.map(({ to, label }) => (
              <li key={label}>
                <Link to={to} className="hover:text-teal-accent">
                  {label}
                </Link>
              </li>
            ))}
      ...
      <div className="max-w-7xl mx-auto mt-16 pt-8 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-6 text-xs text-slate-600">
        <p>© 2024  CCWORLD Consultancy. All rights reserved.</p>
