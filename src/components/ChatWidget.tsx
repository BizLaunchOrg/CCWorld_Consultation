...
import { openOrCreateConversation, sendMessage as sendMessageApi, subscribeToMessages, markRead } from '../lib/chatApi';
...
export function ChatWidget() {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      openOrCreateConversation(null).then((conv) => {
        setConversationId(conv.id);
      });
      markRead(conversationId ?? '').catch(() => {});
    }
  }, [isOpen, clearUnread, conversationId]);
  ...
  useEffect(() => {
    ...
    return unsub;
  }, [conversationId, isOpen, addUnread]);
  ...
  // Demo: simulate support message arriving when panel is closed (increment unread)
  const closedRef = useRef(false);
  useEffect(() => {
    if (!isOpen) {
      closedRef.current = true;
      const t = setTimeout(() => {
        if (closedRef.current) addUnread();
      }, 4000);
  ...
  const sendMessage = useCallback(async () => {
    ...
    // Demo: simulate support reply after 2s; if panel is closed, increment unread
    setTimeout(() => {
  ...
  }, [input, conversationId, addUnread]);
