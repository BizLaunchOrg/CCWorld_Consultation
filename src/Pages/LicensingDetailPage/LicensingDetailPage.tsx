...
function AccordionSection({ section }: { section: ServiceContentSection }) {
  ...
  if (!isChecklist) {
    return (
      <section className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/40 p-6">
  ...
  return (
    <section className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/40 overflow-hidden">
      ...
      {open && section.bullets && (
        <div className="px-6 pb-6 pt-0 border-t border-slate-200 dark:border-slate-800">
  ...
  return (
    <main className="max-w-7xl mx-auto px-6 md:px-20 py-8 pt-16">
      <nav className="flex items-center gap-2 text-sm font-medium text-slate-500 dark:text-slate-400 mb-8">
        ...
        <span className="material-symbols-outlined text-xs">chevron_right</span>
        <Link to="/licensing" className="hover:text-primary">Licensing & Compliance Advisory</Link>
        <span className="material-symbols-outlined text-xs">chevron_right</span>
      ...
      <div className="flex flex-col lg:flex-row gap-12">
        <div className="flex-1 space-y-6">
          <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 p-8">
            <div className="flex items-start gap-4">
              <span className="size-14 rounded-2xl bg-primary/10 flex items-center justify-center text-primary shrink-0">
          ...
          <div className="rounded-2xl border border-primary/20 bg-primary/5 dark:bg-primary/10 p-8 flex flex-col sm:flex-row items-center justify-between gap-6">
            <div>
            ...
            <div className="flex flex-wrap gap-3 shrink-0">
              <Link
                to="/consulting"
                className="px-6 py-3 rounded-xl bg-primary text-white font-black hover:bg-primary/90 transition-all shadow-lg shadow-primary/20"
              >
                Request a Consultating
              </Link>
              <button
                type="button"
                onClick={() => openChat()}
                className="px-6 py-3 rounded-xl border-2 border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white font-bold hover:bg-slate-50 dark:hover:bg-slate-800 transition-all"
              >
                Live Chat
              </button>
            </div>
        ...
        <aside className="w-full lg:w-80 shrink-0">
          <div className="sticky top-32 space-y-6">
            <div className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-900/40 p-6">
